...
| Z.ai | yes | yes | no | JSON object + schema prompt | no | no | no | no | no | 5.3 `low` / `high` / `max`; 5.2 mapped | no | no | Tier B |
...
- Z.ai is Tier B for portable text/tool loops. The adapter preserves `reasoning_content` across streamed and non-streamed tool turns, supports JSON-object structured output with schema prompting and local validation, and exposes only automatic tool selection. GLM-5.3 is currently documented for Coding Plan and requires thinking; the standard Model API catalog and pay-as-you-go pricing still stop at GLM-5.2, so `createZAI()` defaults to the general endpoint and `createZAI({ endpoint: "coding" })` is explicit. Offline contract tests do not certify authenticated availability.
...
- Z.ai:
  - `glm-5.3` requires thinking and accepts `low`, `high`, or `max`; unsupported efforts and disabled thinking fail before network I/O
  - `glm-5.2` maps `none`/`minimal` to disabled thinking, `low`/`medium` to `high`, and `xhigh` to `max`
  - use `endpoint: "coding"` for GLM-5.3 Coding Plan access; the general API remains the default and its documented catalog currently stops at GLM-5.2
