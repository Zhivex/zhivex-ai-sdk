...
The built-in in-memory, SQLite, and Postgres stores implement atomic claims and revision checks. The file store serializes revision CAS across local processes with a private, bounded lock that recovers after the owner exits or the lock becomes stale. Continue to use SQLite or Postgres for durable concurrent workers because file-store leases and broader expired-worker recovery remain local-development facilities. A custom store must implement `claimIdempotencyKey()` to accept idempotent agent inputs.
...
Redaction should happen before data leaves the API process. Keep `includeMessages`, `includeToolInputs`, `includeToolOutputs`, `includeApprovalArguments`, and `includeOutputText` disabled unless the destination is approved for sensitive payloads. `createProductionTraceCollector()` installs common credential and email redaction by default; custom product rules should also cover domain-specific personal or payment data. Preserve metric keys such as `inputTokens`, `outputTokens`, and `totalTokens` so cost dashboards stay useful.
...
For base64 artifacts stored directly through `saveArtifact()`, set `encoding: "base64"`. The SDK calculates missing `size` and `sha256` metadata and validates any values you provide. `saveBinaryArtifact()` likewise rejects a caller-provided SHA-256 that does not match the bytes before persisting.

File-backed binary blob paths are always generated by the SDK under the configured `blobs/` directory. Caller-supplied paths are ignored and persisted metadata that does not match the expected managed path is rejected before read, prune, or delete operations.
