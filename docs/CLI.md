...
Inspection, replay, reporting, comparison, baseline, and gate commands do not execute models or tools. `workflow run` and `workflow eval` import an explicit application-owned local module; the application remains responsible for runners, models, tools, credentials, authorization, and side effects.
