...
const ensurePrivateEnvFile = async (filePath: string, value: string): Promise<void> => {
  await fs.mkdir(path.dirname(filePath), { recursive: true, mode: 0o700 });
  let handle: Awaited<ReturnType<typeof fs.open>>;
  let created = false;
  try {
  ...
  try {
    ...
    if (!stat.isFile()) {
      throw new Error(`Refusing non-file environment path: ${filePath}.`);
    }
    await handle.chmod(0o600);
  } finally {
    await handle.close();
...
${options.provider === "vertex" ? '  projectId: process.env.GOOGLE_CLOUD_PROJECT,\n  location: process.env.VERTEX_LOCATION ?? "global"' : `  apiKey: process.env.${provider.envName}${options.provider === "zai" ? ',\n  endpoint: "coding"' : ""}`}
});
...
const envTemplate = (provider: ProviderTemplateName): string =>
  provider === "vertex"
    ? "GOOGLE_CLOUD_PROJECT=\nVERTEX_LOCATION=global\n# Configure Application Default Credentials before running.\n"
...
${options.provider === "vertex" ? "Set GOOGLE_CLOUD_PROJECT and VERTEX_LOCATION in .env, and configure Google Application Default Credentials (ADC). Project and location alone do not authenticate. Partner models require bearer credentials, not a Google API key. The selected region must serve the chosen model. Doctor reports configuration only; it does not call Google or verify IAM/model access.\n" : ""}
...
const runInitCommand = async (
  ...
  await ensurePrivateEnvFile(path.join(directory, ".env"), envTemplate(rawProvider));
  ...
  printJson(io, {
    ...
    model,
    files: [...files.map(([relativePath]) => relativePath), ".env"]
  });
  return 0;
};
...
const environmentFileCheck = async (directory: string): Promise<DoctorCheck> => {
  ...
  try {
    ...
    if (!stat.isFile() || stat.isSymbolicLink()) {
      return {
        ...
        status: "fail",
        detail: `${envPath} must be a regular file, not a symlink.`
      };
    }
    if (process.platform !== "win32" && (stat.mode & 0o077) !== 0) {
      return {
        name: "env-file-permissions",
        status: "warn",
        detail: `${envPath} is readable by group or other users; use mode 0600.`
      };
    }
    return {
      name: "env-file-permissions",
      status: "pass",
      detail: `${envPath} has private permissions.`
    };
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return {
        name: "env-file-permissions",
        status: "warn",
        detail: `${envPath} was not found; create it with mode 0600 before adding credentials.`
      };
    }
    throw error;
  }
...
const runDoctorCommand = async (
  ...
  checks.push(await environmentFileCheck(directory));
  checks.push({
    name: "package-json",
  ...
  for (const providerName of providerNames) {
    ...
    checks.push({
      ...
      status: providerName === "vertex" ? "warn" : configuredCredential ? "pass" : "warn",
      detail: providerName === "vertex"
        ? configuredCredential
          ? "GOOGLE_CLOUD_PROJECT is set; configure ADC or a bearer token. Authentication, IAM and model access are not verified."
          : "Set GOOGLE_CLOUD_PROJECT and VERTEX_LOCATION, and configure ADC or a bearer token. Credentials are not verified."
        : configuredCredential
...
const readAgentLedgerFromFlags = async (flags: Record<string, string | true>): Promise<AgentRunLedger> => {
  ...
  const includeOutputText = booleanFlag(flags, "include-output-text");
  const ledger = createAgentRunLedger(state, {
    includeTimeline: !booleanFlag(flags, "no-timeline"),
    outputPreviewLength: includeOutputText ? undefined : 0,
    trace: {
      includeOutputText,
      outputPreviewLength: includeOutputText ? undefined : 0
    }
  ...
  if (!includeOutputText) {
    delete (ledger.snapshot as { outputText?: string }).outputText;
    delete (ledger.trace as { outputText?: string }).outputText;
  }
  return ledger;
};
