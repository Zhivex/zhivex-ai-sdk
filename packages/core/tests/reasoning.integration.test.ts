...
const describeReasoningIntegration = reasoningProviders.length ? describe : describe.skip;
  ...
  for (const provider of reasoningProviders) {
    it(`${provider.name} accepts the common reasoning config`, async () => {
      const result = await generateText({
        ...
        ...(provider.omitTemperature || provider.omitTemperatureForReasoning
          ? {}
          : { temperature: provider.temperature ?? 0 }),
        maxTokens: provider.reasoningMaxTokens ?? 64,
